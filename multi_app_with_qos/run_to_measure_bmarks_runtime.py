...
def ensure_data_dir(qos_app):
    dataDirName = 'data_mpstat/%(qos_app)s' % locals()	
...
#@run_thread
def run_mpstat(outfile_name, mpstat_period):
    mpstat_core = 14
    #cmd = ['/bin/bash', '-c', 'setsid', 'taskset', '-c', str(mpstat_core), 'mpstat', '-P', '0,1,2,3,4,5,6,7', str(mpstat_period), '>', outfile_name]
    #cmd = ['taskset', '-c', str(mpstat_core), 'mpstat', '-P', 'ALL', str(mpstat_period)]
    #cmd = 'taskset -c ' + str(mpstat_core) + ' mpstat -P 0,1,2,3,4,5,6,7 ' +  str(mpstat_period) + ' > /tmp/' + outfile_name
    #cmd = 'setsid taskset -c ' + str(mpstat_core) + ' mpstat -P 0,1,2,3,4,5,6,7 ' +  str(mpstat_period) + ' > ' + outfile_name
    cmd = 'taskset -c ' + str(mpstat_core) + ' mpstat -P 0,1,2,3,4,5,6,7 ' +  str(mpstat_period) + ' > ' + outfile_name
    logging.info('Starting mpstat with command: %(cmd)s' % locals())
...
def run_experiment(params, output_base, rep):
    ...
    try: 
        ...
        mpstat_proc = run_mpstat(mpstat_output_filename, 5)
        #threads.append(run_mpstat(add_slot(), mpstat_output_filename, 5))
        #time.sleep(10)
        #sys.exit(1)
        try:
        ...
        os.killpg(os.getpgid(mpstat_proc.pid), signal.SIGTERM)
        with lock:
            for key in procs:
                if procs[key] is not None:
                    driver.kill_process_group(procs[key])
