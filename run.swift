...
runFile = arg("run.file")
tracef("Opening run file: %s", runFile)
runs = readData(runFile)

foreach run in runs {
	file simout <single_file_mapper; file=run.output_name>;
	file simlog <single_file_mapper; file=run.log_name>;
	string application = run.application;
	string interference = run.interference;
	(simout, simlog) = run(application, inteference) 
}
