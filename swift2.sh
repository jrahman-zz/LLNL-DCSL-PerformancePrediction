...
usage() {
	echo "usage: swift2.sh name generator_script [background]"
}
...
CMD="swift -tc.file \"${BASE_PATH}\"/tc.data -sites.file \"${BASE_PATH}/sites.moab.sierra.xml\" \"${BASE_PATH}/run2.swift\" -run.file=\"${RUN_FILE}\""
