...
/usr/apps/python2.7.10/bin/python ${YCSB_DIR}/bin/ycsb run memcached -s -target 31500 -threads 8 -P "${YCSB_DIR}/workloads/${WORKLOAD}" -p recordcount=500000 -p operationcount=3000000 -p "memcached.hosts=127.0.0.1" 2>&1 | egrep "${PATTERN}" > ${ABS_OUTPUT_PATH}
