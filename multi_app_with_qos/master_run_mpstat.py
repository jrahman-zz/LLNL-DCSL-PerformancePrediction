...
@with_file('experiment_list_mpstat', 'r')
def read_experiment_list(f):
    logging.info('Reading experiments from experiment_list_mpstat...')
    experiments = [experiment.strip() for experiment in f if len(experiment.strip()) > 0]
    logging.info('Read %d experiments from experiment_list_mpstat' % len(experiments))
    return experiments

@with_file('completed_experiments_mpstat', 'r+')
...
@with_file('completed_experiments_mpstat', 'w')
