...
void readtxtfile_mat(const char *filename, Coord_MatDouble &A)
{
/*************************************************************************/
...
//  create the arrays to hold the file information:
    ...
    Coord_MatDouble C(maxrow,maxcol,count, val, rowind, colind);
    A = C;
...
void readtxtfile_mat(const char *filename, CompCol_MatDouble &A)
{
    Coord_MatDouble C;
...
void readtxtfile_mat(const char *filename, CompRow_MatDouble &A)
{
    Coord_MatDouble C;
    readtxtfile_mat(filename, C);
    A = C;
...
void writetxtfile_mat(const char *filename, Coord_MatDouble &A)
{
/*************************************************************************/
...
//  Loop through Nonzeros
    for (int j = 0; j < nnz ; j++)
    {
       ...
       fprintf(out_file, "%14d%4s%14d%4s% 20.12E\n", rowp1, "    ", 
                                                     colp1,"    ", A.val(j));
    }
    if (flag == 0)
       fprintf(out_file, "%14d%4s%14d%4s% 20.12E\n", M, "    ",  
...
void writetxtfile_mat(const char *filename, CompCol_MatDouble &A)
{
    FILE *out_file;
    out_file = fopen( filename, "w");
...
void writetxtfile_mat(const char *filename, CompRow_MatDouble &A)
{
    FILE *out_file;
    out_file = fopen( filename, "w");
